...
#define DIR_SPRITES "./materiais/sprites/"
#define DIR_FONTES "./materiais/fontes/"
...
int main() {
    ...
    snprintf(path_personagem, sizeof(path_personagem), "%s%s", DIR_SPRITES,
             "canga.png");
    snprintf(path_background, sizeof(path_background), "%s%s", DIR_SPRITES,
             "background.png");
    snprintf(path_font, sizeof(path_font), "%s%s", DIR_FONTES,
             "FiftiesMovies.ttf");
    snprintf(path_peba, sizeof(path_peba), "%s%s", DIR_SPRITES, "peba.png");
        ...
        // Lógica de Atualização (Física e Movimento)
            ...
            for (int i = 0; i < indice_tatu; i++) {
                if (homem_tatus[i].vivo) {
                    ...
                    if (homem_tatus[i].posx > LARGURA / 2 ||
                        homem_tatus[i].posx > canga.posx) {
                        al_draw_bitmap_region(
                            homem_tatus[i].monstro, png_x, png_y,
                            INIMIGO_LARGURA, INIMIGO_ALTURA,
                            homem_tatus[i].posx, homem_tatus[i].posy, 0);
                    } else if (homem_tatus[i].posx <= LARGURA / 2 ||
                               homem_tatus[i].posx < canga.posx) {
                        al_draw_bitmap_region(
                            homem_tatus[i].monstro, png_x, png_y,
                            INIMIGO_LARGURA, INIMIGO_ALTURA,
