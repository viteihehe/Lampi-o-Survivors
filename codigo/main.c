...
int main()
{
    ...
    if ((retorno_erro = teste(al_init(), "Falha ao inicializar o Allegro")) != 0)
        return retorno_erro;

    // Instalações
    if ((retorno_erro = teste(al_install_keyboard(), "Falha ao instalar o teclado")) != 0)
        return retorno_erro;
    al_init_font_addon();
    al_init_ttf_addon();
    al_init_primitives_addon();
    if ((retorno_erro = teste(al_init_image_addon(), "Falha ao inicializar o addon de imagem")) != 0)
        return retorno_erro;
    ...
    snprintf(path_personagem, sizeof(path_personagem), "%s%s", SPRITES_DIR, "canga.png");
    snprintf(path_background, sizeof(path_background), "%s%s", SPRITES_DIR, "backgroud2.png");
    snprintf(path_font, sizeof(path_font), "%s%s", "./materiais/fontes/", "FiftiesMovies.ttf");
    snprintf(path_peba, sizeof(path_peba), "%s%s", SPRITES_DIR, "peba2_1.png");
    snprintf(path_formiga, sizeof(path_formiga), "%s%s", SPRITES_DIR, "formiga2.png");

    canga.personagem = al_load_bitmap(path_personagem);
    if ((retorno_erro = teste(canga.personagem, "Falha ao carregar a imagem do personagem. Verifique o caminho e as dependências PNG.")) != 0)
        return retorno_erro;

    sprite_tatu = al_load_bitmap(path_peba);
    if ((retorno_erro = teste(sprite_tatu, "Falha ao carregar o sprite do homem peba.")) != 0)
        return retorno_erro;

    sprite_formiga = al_load_bitmap(path_formiga);
    if ((retorno_erro = teste(sprite_formiga, "Falha ao carregar o sprite da formiga")) != 0)
        return retorno_erro;

    ALLEGRO_BITMAP *background = al_load_bitmap(path_background);
    if ((retorno_erro = teste(background, "Falha ao carregar a imagem de fundo.")) != 0)
        return retorno_erro;

    // Flags corrigidas (0 em vez de NULL)
    font = al_load_font(path_font, 20, 0);
    if ((retorno_erro = teste(font, "Falha ao carregar a fonte. Verifique o caminho e se o Allegro TTF está instalado.")) != 0)
        return retorno_erro;

    fila_eventos = al_create_event_queue();
    if ((retorno_erro = teste(fila_eventos, "Falha ao criar a fila de eventos")) != 0)
        return retorno_erro;
    ...
    while (!fim)
    {
        ...
        else if (ev.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            if (counts - ultimo_spawn_formiga >= cooldown_formiga && indice_formiga < LIMITE)
            {
            ...
            for (int i = 0; i < indice_formiga; i++)
            {
                if (formigas[i].vivo)
                {
                    ...
                    if (counts - formigas[i].ultimo_disparo >= disparo_cooldown && cont_disparo < MAX_BALAS)
                    {
